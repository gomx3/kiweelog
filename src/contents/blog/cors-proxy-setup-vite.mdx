---
title: '[Vite·React] Proxy 설정으로 CORS 문제 해결하기 (.env 활용)'
pubDate: 2025-05-31
description: '개발 환경에서 CORS 오류를 피하기 위해 Vite의 Proxy와 환경 변수를 설정하는 방법을 정리했습니다.'
author: '김소원'
image:
    url: '/images/cors-proxy-setup-vite/kiwi.png'
    alt: '의미 없는 키위'
...
우리는 동일 출처 정책(Same Origin Policy, SOP)을 위반하고 있던 것이다. SOP는 한 Origin에서 로드된 문서나 스크립트가 다른 Origin의 리소스와 상호 작용하는 것을 제한하는 보안 메커니즘이다.

즉, 웹 브라우저의 보안을 위해 프로토콜, 호스트, 포트가 동일한 서버에서만 ajax 요청을 주고 받을 수 있도록 한 정책이다.
